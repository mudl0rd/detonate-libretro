...
public:
    ...
    {
        int sample_r = 44100;
        std::vector<uint8_t> dat = load_data(filename);
        if (!m4p_LoadFromData(dat.data(), dat.size(), sample_r, sample_r/60))
            return false;
        *samplerate = sample_r;
        isplaying2 = true;
        m4p_PlaySong();
    ...
    void mix(float *&buffer_samps, unsigned &count)
    {
        ...
        int16_t temp_bufferint[count * 4 * sizeof(int32_t)] = {0};
        unsigned temp_samples = 0;
        int status = 0;
        if (isplaying2)
        {
        again:
            status = m4p_GenerateSamples(temp_bufferint, count);
            if (status = 0)
            {
                isplaying2 = false;
                return;
            }
            conv2float(temp_buffer, (uint8_t *)temp_bufferint, count * 2, sampfmt::P16);
            temp_samples = count;
        }
